...
def exchange_token(code):
    ...
    print("Pat Add debug strava request (in exchange_token)")
...
blueprint.route('/dashtry', methods = ['GET'])
@blueprint.route('/dashtry.html', methods = ['GET'])
def dashtry():
    # if count is in the args list then we return the number of activities 
    count=request.args.get('count')
    if count != None :
        data=ga.get_activities(session['uid'])
    ...
    data=ga.get_activities(session['uid'], limit,offset)
    print("Pat Add data=",data)
    return jsonify(data) 
...
def ptry():
    ...
    if offset == None:
        offset=""
    return render_template('home/try.html',limit=limit,offset=offset)
...
def pat():
    ...
    if offset == None:
        offset=""
    count=len(ga.get_activities(session['uid'])) 
...
def dashboard():
    ...
    activities = ga.get_activities(session['uid'])
    ascensions =  ga.get_ascensions(5,session['uid'],starred)
    last3activities = json.dumps([row[0:9] for row in ga.get_last_3_activities(session['uid'])])
    last3ascensions = json.dumps([row[0:5] for row in ga.get_last_3_ascensions_2(1000,3,session['uid'])])
    ...
    weekactivities=json.dumps([row[0:8] for row in ga.get_activities_in_interval(session['uid'],str(week_before),str(now))])
    weekbeforeactivities=json.dumps([row[0:8] for row in ga.get_activities_in_interval(session['uid'],str(two_week_before),str(week_before))])
    monthactivities=json.dumps([row[0:8] for row in ga.get_activities_in_interval(session['uid'],str(month_before),str(now))])
    monthbeforeactivities=json.dumps([row[0:8] for row in ga.get_activities_in_interval(session['uid'],str(two_month_before),str(month_before))])
    yearactivities=json.dumps([row[0:8] for row in ga.get_activities_in_interval(session['uid'],str(year_before),str(now))])
    yearbeforeactivities=json.dumps([row[0:8] for row in ga.get_activities_in_interval(session['uid'],str(two_year_before),str(year_before))])
...
def synching():
    ...
    for ac in a :
        ...
        try : 
            ...
            print("Pat Add delete after addactivity")
            rs=ga.gse(ac,ra,session['token'])
            if rs == "quota exceeded":
                print("Pat Add on a depasse le quota")
                break
            print("Pat Add delete after gse")
